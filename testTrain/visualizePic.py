...
def save_generated_images(generated_images, epoch):
    for batch in range(generated_images.shape[0]):
        for time in range(generated_images.shape[1]):
            ...
            # 创建文件夹
            file = "E:/DGMR/trainResult/train/epoch%d" % (epoch + 1)
            mkdir(file)
            # 保存图片
            plt.savefig('E:/DGMR/trainResult/train/epoch%d/batch%dTime%d.jpg' % (epoch + 1, batch, time))


def save_raw_images(images):
    for batch in range(images.shape[0]):
        for time in range(images.shape[1]):
            ...
            ax.imshow(numpy_image, vmin=0, vmax=10, cmap="jet")
            # 创建文件夹
            file = "E:/DGMR/raw_images/batch%d" % batch
            mkdir(file)
            # 保存图片
            plt.savefig('E:/DGMR/raw_images/batch%d/time%d.jpg' % (batch, time))
